(() => {
  ...
  const MAX_SCROLL_SPEED = 2.0; // px per ms
  ...
  function checkSuspicious() {
    ...
    console.log("[Detection] Linearity:", linearity.toFixed(3), "| Typing:", suspiciousTyping, "| Scroll speed suspicious:", suspiciousScroll);

    if (suspiciousMouse) console.log("Suspicious: Mouse path too straight");
    if (suspiciousTyping) console.log("Suspicious: Typing speed/irregularity");
    if (suspiciousScroll) console.log("Suspicious: Scrolling too fast");
